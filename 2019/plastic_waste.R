...
pop_mismanaged_waste %>% filter(!is.na(coastal_population)) %>% 
   skim()
gdp_mismanaged_waste %>% filter(
   !is.na(per_capita_mismanaged_plastic_waste_kilograms_per_person_per_day)) %>% 
   skim()
gdp_waste %>% filter(
...
waste <- waste_join %>%
   ...
   select(
      ...
      gdp, gdp_pc,
      plastic_waste_kg_pc_pd, 
      total_plastic_waste_kg_pd, 
      mismanaged_plastic_waste_tonnes,
      mismanaged_plastic_waste_kg_pc_pd, 
      total_mismanaged_plastic_waste_kg_pd
      )
